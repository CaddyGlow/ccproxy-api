...
class TestSmokeTests:
    ...
    async def test_copilot_chat_completions_stream(self, client: httpx.AsyncClient) -> None:
        ...
        event_count = 0
        async with client.stream("POST", "/copilot/v1/chat/completions", json=payload, headers=headers) as response:
            assert response.status_code == 200
    ...
    async def test_copilot_responses_stream(self, client: httpx.AsyncClient) -> None:
        ...
        event_count = 0
        async with client.stream("POST", "/copilot/v1/responses", json=payload, headers=headers) as response:
            assert response.status_code == 200
            assert "text/event-stream" in response.headers.get("content-type", "")
    ...
    async def test_codex_chat_completions_stream(self, client: httpx.AsyncClient) -> None:
        ...
        event_count = 0
        async with client.stream("POST", "/api/codex/v1/chat/completions", json=payload, headers=headers) as response:
            assert response.status_code == 200
            assert "text/event-stream" in response.headers.get("content-type", "")
    ...
    async def test_codex_responses_stream(self, client: httpx.AsyncClient) -> None:
        ...
        event_count = 0
        async with client.stream("POST", "/codex/responses", json=payload, headers=headers) as response:
            assert response.status_code == 200
            assert "text/event-stream" in response.headers.get("content-type", "")
