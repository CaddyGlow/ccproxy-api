...
from typing import Any, AsyncIterator, Callable, Dict, Iterable, Tuple
...
def stream_paths(name: str) -> Tuple[Path, Path]:
    _ensure_dir()
    return (MOCKS_DIR / f"{name}.mock.stream.jsonl", MOCKS_DIR / f"{name}.mock.stream.headers.json")


def save_normal_mock(name: str, status: int, headers: Dict[str, str], body: Any) -> None:
...
def make_mock_middleware(routes: Dict[Tuple[str, str], str]) -> Callable[[Request, Callable[[Request], Any]], Any]:
    ...
    async def middleware(request: Request, call_next: Callable[[Request], Any]) -> Response:
        ...
        if not record:
            ...
            if path.endswith("/chat/completions") or path.endswith("/responses"):
                ...
                headers = hop_by_hop_filter({k: v for k, v in data.get("headers", {}).items()})
            ...
            headers = hop_by_hop_filter({k: v for k, v in hdrs.get("headers", {}).items()})
            ...
            return StreamingResponse(gen(), status_code=status, headers=headers, media_type=headers.get("content-type", "text/event-stream"))

        # Record mode
        response = await call_next(request)
        # Clone/capture body. For JSON, read and store; for streams, read full text.
        raw = await response.aread()
        headers = hop_by_hop_filter({k: v for k, v in dict(response.headers).items()})
        ...
        if "text/event-stream" in content_type:
            async def regen() -> AsyncIterator[bytes]:
                for ln in lines:
                    yield (ln + "\n").encode()
            return StreamingResponse(regen(), status_code=status, headers=headers, media_type=content_type)
        else:
            return Response(content=raw, status_code=status, headers=headers, media_type=content_type)
