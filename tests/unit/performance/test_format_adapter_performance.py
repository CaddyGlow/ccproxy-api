...
class TestFormatAdapterPerformance:
    ...
    async def test_large_scale_registration_performance(self, registry):
        ...
        for plugin_idx in range(num_plugins):
            ...
            for adapter_idx in range(adapters_per_plugin):
                spec = FormatAdapterSpec(
                    ...
                    adapter_factory=lambda: BenchmarkAdapter(
                        f"adapter_{plugin_idx}_{adapter_idx}"
                    ),
                    priority=plugin_idx,
                )
                specs.append(spec)
            ...
            manifest = PluginManifest(
                name=f"plugin_{plugin_idx}", version="1.0.0", format_adapters=specs
            )
        ...
        assert registration_time < 5.0, (
            f"Registration took {registration_time:.2f}s for {total_adapters} adapters"
        )
        assert finalization_time < 10.0, (
            f"Finalization took {finalization_time:.2f}s for {total_adapters} adapters"
        )
        ...
        print(
            f"Performance: Registered {total_adapters} adapters in {registration_time:.3f}s"
        )
        print(
            f"Performance: Finalized {total_adapters} adapters in {finalization_time:.3f}s"
        )
    ...
    async def test_conflict_resolution_performance(self, registry):
        ...
        for conflict_idx in range(num_conflicts):
            ...
            for plugin_idx in range(plugins_per_conflict):
                spec = FormatAdapterSpec(
                    ...
                    adapter_factory=lambda: BenchmarkAdapter(
                        f"conflict_{conflict_idx}_{plugin_idx}"
                    ),
                ...
                await registry.register_from_manifest(
                    manifest, f"conflict_plugin_{conflict_idx}_{plugin_idx}"
                )
        ...
        assert resolution_time < 2.0, (
            f"Conflict resolution took {resolution_time:.2f}s for {num_conflicts} conflicts"
        )
        ...
        print(
            f"Performance: Resolved {num_conflicts} conflicts in {resolution_time:.3f}s"
        )
    ...
    async def test_concurrent_registration_safety(self, registry):
        ...
        async def register_plugin(plugin_id: int):
            ...
            await registry.register_from_manifest(
                manifest, f"concurrent_plugin_{plugin_id}"
            )

        # Register plugins concurrently
        start_time = time.perf_counter()
        await asyncio.gather(*[register_plugin(i) for i in range(num_concurrent)])
        ...
        print(
            f"Performance: {num_concurrent} concurrent registrations in {concurrent_time:.3f}s"
        )
    ...
    async def test_memory_usage_with_large_registry(self, registry):
        ...
        initial_size = sys.getsizeof(registry._adapters) + sys.getsizeof(
            registry._adapter_specs
        )
        ...
        for i in range(num_adapters):
            ...
            manifest = PluginManifest(
                name=f"memory_plugin_{i}", version="1.0.0", format_adapters=[spec]
            )
        ...
        final_size = sys.getsizeof(registry._adapters) + sys.getsizeof(
            registry._adapter_specs
        )
        memory_per_adapter = (final_size - initial_size) / num_adapters

        # Memory usage should be reasonable
        assert memory_per_adapter < 1000, (
            f"Memory per adapter: {memory_per_adapter:.0f} bytes"
        )
    ...
    def test_format_adapter_spec_creation_performance(self):
        ...
        assert creation_time < 1.0, (
            f"Creating {num_specs} specs took {creation_time:.2f}s"
        )
        ...
        print(
            f"Performance: Created {num_specs} FormatAdapterSpecs in {creation_time:.3f}s"
        )
...
class TestConflictResolutionValidation:
    ...
    async def test_priority_conflict_resolution_correctness(self, priority_registry):
        ...
        for i, priority in enumerate(priorities):
            ...
            manifest = PluginManifest(
                name=f"priority_plugin_{i}", version="1.0.0", format_adapters=[spec]
            )

            await priority_registry.register_from_manifest(
                manifest, f"priority_plugin_{i}"
            )
    ...
    async def test_fail_fast_conflict_detection(self, fail_fast_registry):
        ...
        for i in range(3):
            ...
            manifest = PluginManifest(
                name=f"fail_fast_plugin_{i}", version="1.0.0", format_adapters=[spec]
            )

            await fail_fast_registry.register_from_manifest(
                manifest, f"fail_fast_plugin_{i}"
            )
    ...
    async def test_complex_conflict_scenario(self, priority_registry):
        ...
        for from_fmt, to_fmt, priorities in conflicts:
            ...
            for priority in priorities:
                ...
                await priority_registry.register_from_manifest(
                    manifest, f"complex_plugin_{plugin_counter}"
                )
                plugin_counter += 1
