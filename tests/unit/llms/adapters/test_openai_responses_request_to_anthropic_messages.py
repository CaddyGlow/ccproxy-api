...
async def test_openai_responses_request_to_anthropic_messages_adapt_stream_minimal() -> (
    ...
    assert any(
        isinstance(e, ResponseOutputTextDeltaEvent)
        and e.type == "response.output_text.delta"
        and e.delta == "Hi"
        for e in res_events
    ), (
        f"No delta event with 'Hi' found in events: {[e.type if hasattr(e, 'type') else str(e) for e in res_events]}"
    )

    assert any(
        isinstance(e, ResponseCompletedEvent) and e.type == "response.completed"
        for e in res_events
    ), (
        f"No completed event found in events: {[e.type if hasattr(e, 'type') else str(e) for e in res_events]}"
    )
...
async def test_complex_input_parameter_handling() -> None:
    ...
    # The current implementation only extracts text, so content will be a string
    assert isinstance(anth_mixed.messages[0].content, str)
    assert "Look at this" in anth_mixed.messages[0].content
...
async def test_complex_nested_request_structures() -> None:
    ...
    assert getattr(anth_comprehensive.thinking, "budget_tokens", 0) == 1024
    assert anth_comprehensive.max_tokens > 1024
    assert anth_comprehensive.temperature == 1.0  # Overridden by thinking
