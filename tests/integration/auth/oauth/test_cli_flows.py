...
class TestCLICallbackServer:
    ...
    async def test_callback_server_port_bind_error(self) -> None:
        ...
        with patch("uvicorn.Server") as mock_server_class:
            ...
            with pytest.raises(
                PortBindError, match="Failed to start callback server on port 8080"
            ):
                await server.start()
    ...
    async def test_wait_for_callback_success(self) -> None:
        ...
        with patch.object(server, "wait_for_callback", side_effect=mock_wait):
            result = await server.wait_for_callback("test_state", timeout=0.1)
    ...
    async def test_wait_for_callback_timeout(self) -> None:
        ...
        with pytest.raises(
            asyncio.TimeoutError, match="No OAuth callback received within 0.1 seconds"
        ):
            await server.wait_for_callback("test_state", timeout=0.1)
