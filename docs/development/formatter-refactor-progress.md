...
- End-to-end unit suite (`./Taskfile test-unit`) and lint/typing (`./Taskfile check`) passing after each phase.
...
- Introduced lightweight stream wrapper functions inside the facade modules so adapters remain patchable and kept `./Taskfile check` and `./Taskfile test-unit` green after the split.
...
- Kept streaming tests green (`./Taskfile check`, `./Taskfile test-unit`) while simplifying future extraction of additional stream helpers.
...
- Verified the refactor with `./Taskfile check` and `./Taskfile test-unit`, ensuring parity across all streaming converters touched so far.
...
- Confirmed parity via `./Taskfile check` and `./Taskfile test-unit`, aligning all three provider pairs on the common streaming scaffolding.
...
- Verified the migration with `./Taskfile check` and `./Taskfile test-unit`, completing the cross-provider adoption of the shared streaming infrastructure.
...
- Confirmed helper extraction with `./Taskfile check` and `./Taskfile test-unit`.
