...
- Run `make dev-install` to sync the new dependency graph and pre-commit hooks.
...
3. Run `make test-unit` to confirm `validate_chains` and `validate_stream_pairs` succeed during startup.
...
- `make ci` passes (ruff, mypy, pytest) with your custom plugins enabled.
