...
def create_copilot_router(adapter: CopilotAdapter) -> APIRouter:
    """Create FastAPI router for Copilot endpoints.

    Args:
        adapter: Copilot adapter instance
    ...
    async def list_models() -> JSONResponse:
        ...
        try:
            ...
            fallback_path = (
                pathlib.Path(__file__).parent / "data" / "copilot_fallback.json"
            )
    ...
    async def get_token_status() -> JSONResponse:
        ...
        try:
            ...
            if not adapter.oauth_provider:
                raise HTTPException(
                    status_code=503, detail="OAuth provider not available"
                )
