...
class BaseHTTPAdapter(ABC):
    ...
    def __init__(self, auth_manager, http_pool_manager, **kwargs):
    ...
    async def handle_request(self, request: Request) -> Response | StreamingResponse:
        ...
        response = await self._execute_http_request(
            ctx.metadata.get("method", request.method), target_url, prepared_headers, prepared_body
        )
        
        # Step 5: Provider-specific response processing
        return await self.process_provider_response(response, ctx.metadata.get("endpoint", "/"))
    ...
    async def _execute_format_chain(
        ...
        for i in range(len(format_chain) - 1):
            ...
            try:
                ...
                ctx.log_event("format_chain_step_completed",
                            from_format=from_format, to_format=to_format, step=i+1)
                            
            except Exception as e:
                ctx.log_event("format_chain_step_failed",
