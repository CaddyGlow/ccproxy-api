            ...
            for hook in hooks:
                try:
                    ...
                    self._logger.error(
                        ...
                        hook=hook.name,
                        event=task.context.event.value if hasattr(task.context.event, "value") else str(task.context.event),
                        error=str(e),
                        task_id=task.task_id,
                    )
        except Exception as e:
            self._logger.error("hook_task_execution_failed", error=str(e), task_id=task.task_id)
