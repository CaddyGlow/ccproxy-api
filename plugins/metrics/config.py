...
class MetricsConfig(BaseModel):
    ...
    pushgateway_enabled: bool = Field(
        default=False,
        description="Enable Pushgateway integration for batch metrics"
    )
    
    pushgateway_url: str | None = Field(
        default=None,
        description="Pushgateway URL (e.g., http://localhost:9091)"
    )
    
    pushgateway_job: str = Field(
        default="ccproxy",
        description="Job name for Pushgateway"
    )
    
    pushgateway_push_interval: int = Field(
        default=60,
        description="Interval in seconds between pushes to Pushgateway"
    )
    
    # Collection settings
    collect_request_metrics: bool = Field(
        default=True,
        description="Collect request/response metrics"
    )
    
    collect_token_metrics: bool = Field(
        default=True,
        description="Collect token usage metrics"
    )
    
    collect_cost_metrics: bool = Field(
        default=True,
        description="Collect cost metrics"
    )
    ...
    collect_pool_metrics: bool = Field(
        default=True,
        description="Collect connection pool metrics"
    )
